// Package cli builds the smashingmag command tree on top of the smashingmag library.
package cli
...
type App struct {
	client *smashingmag.Client
	cfg    smashingmag.Config
...
func Root() *cobra.Command {
	app := &App{cfg: smashingmag.DefaultConfig()}

	root := &cobra.Command{
		Use:   "smashingmag",
		Short: "Browse Smashing Magazine web design articles",
		Long: `smashingmag reads Smashing Magazine through its public RSS feed.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

smashingmag is an independent tool and is not affiliated with Smashing Magazine.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = smashingmag.NewClient(a.cfg)
	return nil
}
