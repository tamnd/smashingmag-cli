...
describes what users get. Pick a format with `-o`, or let smashingmag choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
smashingmag <command> -o table   # aligned columns for reading
smashingmag <command> -o jsonl   # one JSON object per line, for piping
smashingmag <command> -o json    # a single JSON array
smashingmag <command> -o csv     # spreadsheet friendly
smashingmag <command> -o tsv     # tab-separated
smashingmag <command> -o url     # just the URL column
smashingmag <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
smashingmag <command> --fields id,title,url
```
...
```bash
smashingmag <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
smashingmag <command>            # a table, because this is a terminal
smashingmag <command> | wc -l    # JSONL, because this is a pipe
```
